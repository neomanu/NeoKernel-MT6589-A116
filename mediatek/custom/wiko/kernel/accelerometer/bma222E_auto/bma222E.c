...
/*----------------------------------------------------------------------------*/
static void bma222E_power_suspend(struct power_suspend *h) 
{
	struct bma222E_i2c_data *obj = container_of(h, struct bma222E_i2c_data, power_drv);   
	int err;
	GSE_FUN();    
...
/*----------------------------------------------------------------------------*/
static void bma222E_late_resume(struct power_suspend *h)
{
	struct bma222E_i2c_data *obj = container_of(h, struct bma222E_i2c_data, power_drv);         
	int err;
	GSE_FUN();
...
static int bma222E_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->power_drv.suspend  = bma222E_power_suspend,
	obj->power_drv.resume   = bma222E_late_resume,    
